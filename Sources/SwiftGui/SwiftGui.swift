...
public class MyApplication: Application {
  ...
  func helloButton() -> Button {
    Button("Hello!").handler {
      print("Hello!")
    }
  }
  ...
  func toastOverlay(win: Window) -> ToastOverlay {
    ...
    let carousel = Carousel().append(
      StatusPage().title("Hello").description("Page 1")
        .hexpand()
        .frame(minHeight: 300)
    ).append(
      StatusPage().title("World").description("Page 2")
        .hexpand()
    )
    ...
    toastOverlay = ToastOverlay (
      Box(horizontal: false).append(
        ...
        StatusPage().title("Hello").description("World").icon(.default(icon: .softwareUpdateUrgent)).child(
          Box(horizontal: false, spacing: 5).append(
            MarkupLabel("<b>Foo</b> Bar")
          ).append(
            Box(horizontal: false, spacing: 5).append(
              Box(spacing: 5).append(
                ...
                Button("Toast").handler {
                  let toast = Toast("Quit this <i>application!</i>").buttonLabel("Actions").buttonHandler {
                    let messageDialog = MessageDialog(parent: win, heading: "Hello, world!", body: "Some body text").response(id: "cancel", label: "Cancel", type: .closeResponse) {
                      print("Cancel")
                    }.response(id: "about", label: "About"){
                      self.aboutWindow(win: win).show()
                    }.response(id: "quit", label: "Quit", appearance: .destructive, type: .defaultResponse) {
                      print("Quit")
                      self.quit()
                    }.child(
                      Avatar(size: 100).fallbackIcon(.default(icon: .dialogWarning))
                    )
                    messageDialog.show()
                  }
  ...
  func createWindow() -> Window {
    ...
    let contentView = TabOverview().createTabHandler {
      createTab()
    }
    ...
    _ = contentView.add(
      box.append(
          ...
          Button(icon: .default(icon: .tabNew)).handler {
            _ = createTab()
          }
        ).packEnd(
          TabButton(view: tabView).handler {
            contentView.showOverview()
          }
        )
      ).append(
        TabBar(view: tabView)
  ...
  public override func onActivate() {
    ...
    preferencesWindow.add(
      page: .init(name: "Hello", icon: .default(icon: .daytimeSunrise), description: "Hello world?").add(
        group: group.add(
          ...
          Button("Add").handler {
            _ = group.add(
              ActionRow(title: String(Int.random(in: 0...1000)), subtitle: String(Int.random(in: 0...100000)))
            )
          }.padding()
        )
      ).add(
        group: .init(name: "Another Group", description: "Description").add(
          ExpanderRow(title: "ExpanderRow", subtitle: "Description").addRow(
            ...
            ActionRow(title: "Row 2", subtitle: "Description")
          )
        )
      )
    )
    ...
    preferencesWindow.add(page: .init(name: "World", icon: .default(icon: .faceCool), description: "Cool!").add(
